"""Command-line adapter for WaveQuant research and operations use cases."""
...
from wavequant.application.analytics.research import make_signals, run_research, save_result
...
def main() -> None:
    ...
    if args.command in ("system-run", "system-status", "system-backup", "system-restore", "system-alert-ack"):
        from wavequant.application.governance.operations import OperationsConfig, run_operations, operations_status
        from wavequant.infrastructure.filesystem.backup_bundle import create_bundle, restore_bundle
        from wavequant.infrastructure.persistence.operational_store import workspace_lock, acknowledge_alert, now
        from wavequant.infrastructure.persistence.event_store import EventStore
    ...
    if args.command == "strategy-evidence":
        from wavequant.interfaces.research_tools.strategy_evidence import run_strategy_evidence
    ...
    if args.command == "platform-audit":
        from wavequant.application.governance.platform_audit import run_platform_audit
    ...
    if args.command == "paper-demo":
        from wavequant.application.governance.platform_audit import run_paper_demo
