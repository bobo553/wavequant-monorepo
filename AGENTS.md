...
Git 代码开发默认在独立 worktree 中进行；多个任务分别使用独立分支和 worktree。特性分支命名为 `feat-中文名`，会话默认使用分支名；用户手动修改会话名后保留用户名称。每次提交的标题准确描述改动且少于 35 个字符；提交后逐个同步主分支、检查、复核、合并并推送 `main`。本仓库主分支是 `main`。完整流程及个人 Codex 全局规则的版本化副本见 `docs/agent/codex-global-AGENTS.md` 的“Git 多任务开发与本地合并流程”。该副本不会自动替换各开发者本机的 `~/.codex/AGENTS.md`。
...
- 特性分支提交与合并后推送 `main` 按本文件工作流执行；未经用户明确要求不得创建 PR、发布或部署。
