                    ...
                    >
                        ...
                        {workspace.dialog.kind === "settings" ? (
                            <div className="space-y-5 p-5">
                                <Setting label="主题" description="切换深色或浅色工作区">
                                    <select
                                        aria-label="主题"
                                        value={workspace.theme}
                                        onChange={(event) =>
                                            workspace.setTheme(event.target.value === "light" ? "light" : "dark")
                                        }
                                ...
                                <Setting label="涨跌配色" description="替代配色仍保留正负号与文字">
                                    <select
                                        ...
                                        onChange={(event) =>
                                            workspace.setPalette(
                                                event.target.value === "accessible" ? "accessible" : "classic",
                                            )
                                        }
                                        className="border-border bg-background h-9 rounded-md border px-3"
                                    >
                                ...
                                <p className="rounded-lg border border-amber-400/20 bg-amber-400/8 p-3 text-xs text-amber-300">
                                    设置保存在浏览器的 wavequant.market.v2 空间，不上传到服务器。
                                </p>
                            </div>
                        ) : null}
                    </section>
