...
- MONOREPO-171：V3 巨量高开反包下跌（成交量创前 10 日新高且≥均量 2 倍、开盘越前高、收盘破前低、阴线实体≥开盘 5%）当日模拟清余仓，优先于普通放量下跌及目标阶段部分减仓。国芳 2020-08-14 真实 API 与 Chromium 清仓验证通过；Core 139 项相关测试、Web 14+150 项单测、lint、类型、构建及 Harness 通过。

- MONOREPO-170：普通 A 浪入场时冻结一饱已达、二吐未达的分类；C 浪触及等浪目标后放量长上影按累计 80% 目标减仓，异常后首次前收下跌当日清余仓。国芳 2020-06-02 减仓、06-05 清仓真实 API 与 Chromium 通过；Core 55 项相关测试、Web 14+150 项单测、lint、类型、构建及 Harness 通过。
...
- MONOREPO-153：每日涨停天梯接入AkShare东方财富真实池，支持日期选择/前后日/刷新/保存日期、分组/筛选/导出；总览同步真实预览，天梯隐藏演示时轴及状态。真实09-21 103只、09-18 78只。API85+4（1跳过）、Web10+132、2条浏览器和相关类型/lint/构建通过。
