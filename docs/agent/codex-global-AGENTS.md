...
- 适用于有 `main` 分支的 Git 项目中的代码开发任务。新建开发会话时，从最新的本地 `main` 为该任务建立独立的 Git worktree 和独立的 `feature/<任务名>` 分支；多个任务并行时，每个会话各用一个 worktree 和一个分支，不能共用工作目录或开发分支。不要在正在使用的主工作目录中为并行任务反复切换分支。
...
- 各任务提交后，按顺序将 feature 分支合并到本地 `main`。合并每一个任务前，先让该分支同步当时最新的本地 `main`，处理冲突并重新运行受影响的检查；复核该分支相对于 `main` 的最终差异后，本地执行 `git merge`。第二个及后续任务必须基于前一个任务合并后的 `main` 重复同步、冲突处理、验证和复核。
- 只有提交、检查和复核都完成且合并条件满足时才执行本地合并。合并后确认 `main` 状态，并报告各任务的分支、提交、检查结果及合并结果。不要自动推送远端，也不要使用强制推送。
