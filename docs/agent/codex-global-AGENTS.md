...
- 在各自的特性分支完成实现、与改动直接相关的检查和代码复核，再提交该任务的改动。验证节奏遵循上文“开发验证节奏”；检查未通过时先修复，无法通过时说明原因，不合并。
- 各任务提交后，按顺序将特性分支合并到本地 `main`。合并每一个任务前，先让该分支同步当时最新的本地 `main`，处理冲突并重新运行受影响的检查；复核该分支相对于 `main` 的最终差异后，本地执行 `git merge`。第二个及后续任务必须基于前一个任务合并后的 `main` 重复同步、冲突处理、验证和复核。
