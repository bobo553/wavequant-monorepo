...
- Git 代码开发默认使用独立 worktree。新建 Codex 会话且可选择运行环境时，优先选择 `Worktree`（或 `/worktree`），并选择目标基线分支；通过任务创建工具启动新会话时，指定 worktree 环境。若会话已经在 `Local` 启动且尚无专属 worktree，先创建一次独立 Git worktree，后续文件修改和 Git 命令都在其中执行。用户明确要求直接操作当前检出时，遵从该次要求。非 Git 项目及只读问答不需要 worktree。`AGENTS.md` 只能约束执行流程，不能改变 Codex 界面的新任务默认选项。
- 适用于有 `main` 分支的 Git 项目中的代码开发任务。新建开发会话时，从最新的本地 `main` 为该会话建立一个独立的 Git worktree 和分支，特性分支命名为 `feat-中文名`（`feat-` 后使用简明中文会话名）。同一会话内的后续功能、修复和规则修改继续使用该分支，不按功能另建分支；分支合并进 `main` 后仍保留，开始下一项工作前先同步最新的本地 `main`。不同会话并行时各用自己的 worktree 和分支，不能共用工作目录或开发分支。不要在正在使用的主工作目录中为并行会话反复切换分支。
...
- 各任务提交后，按顺序将特性分支合并到本地 `main`。合并每一个任务前，先让该分支同步当时最新的本地 `main`，处理冲突并重新运行受影响的检查；复核该分支相对于 `main` 的最终差异后，使用 `git merge --no-ff -m "chore: 合并<任务内容>"` 生成标题少于 35 个字符的合并提交。第二个及后续任务必须基于前一个任务合并后的 `main` 重复同步、冲突处理、验证和复核。
- 只有提交、检查和复核都完成且合并条件满足时才执行本地合并。合并后确认 `main` 状态并报告分支、提交、检查与合并结果；不要自动推送远端。仅在用户明确要求时推送，不强制推送或改写远端历史。
