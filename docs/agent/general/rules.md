...
3. 按变更风险手动运行受影响的定向测试；未运行项记录为待验证，不作为每次提交的同步门禁。
4. 跨包改动按需运行消费者构建或不含测试的快速门禁 `pnpm verify:quick`。
5. 完整 `pnpm verify` 含自动化测试，可在独立验证时执行；GitHub Actions 在推送或 PR 后运行单元测试及静态检查。本地提交不运行钩子检查。
